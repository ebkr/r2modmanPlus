<template>
    <div>
                ...
                <div class='card'>
                    <header class="card-header">
                        <p class='card-header-title' v-if="selectedThunderstoreMod !== null">Select a version of
                            {{selectedThunderstoreMod.getName()}} to download</p>
                    </header>
                ...
                <div v-if="dependencyListDisplayType !== 'view'">
                    <ul class="list">
                        ...
                        <li class="list-item" v-for='(key, index) in getDependantList(selectedManifestMod)'
                            :key='`dependant-${index}`'>{{key.getName()}}
                        </li>
                    </ul>
                </div>
                <div v-if="dependencyListDisplayType === 'view'">
                    <div v-if="getDependencyList(selectedManifestMod).size > 0">
                        ...
                        <ul class="list">
                            <li class="list-item" v-for='(key, index) in getDependencyList(selectedManifestMod)'
                                :key='`dependency-${index}`'>{{key.getName()}}
                            </li>
                        </ul>
                    </div>
        ...
        <modal v-show="fixingPreloader" @close-modal="closePreloaderFixModal">
            ...
            <template v-slot:footer>
                <button v-if="dependencyListDisplayType === 'view'" class="button is-info"
                        @click="closePreloaderFixModal()">I understand
                </button>
            </template>
        </modal>
                    ...
                    <ul class="menu-list">
                        <li>
                            <a @click="() => {view = 'installed'; searchFilter = ''}"
                               :class="[view === 'installed' ? 'is-active' : '']"><i class="fas fa-folder"/>&nbsp;&nbsp;Installed
                                ({{localModList.length}})</a>
                        </li>
                        <li>
                            <a @click="() => {view = 'online'; searchFilter = ''}"
                               :class="[view === 'online' ? 'is-active' : '']"><i class="fas fa-globe"/>&nbsp;&nbsp;Online
                                ({{thunderstoreModList.length}})</a>
                        </li>
                    </ul>
                    <p class='menu-label'>Other</p>
                    <ul class='menu-list'>
                        <li><a @click="openConfigEditor()" :class="[view === 'config_editor' ? 'is-active' : '']"
                               v-if="!settings.legacyInstallMode"><i class="fas fa-edit"/>&nbsp;&nbsp;Config Editor</a>
                        </li>
                        <li><a @click="view = 'settings'" :class="[view === 'settings' ? 'is-active' : '']"><i
                                class="fas fa-cog"/>&nbsp;&nbsp;Settings</a></li>
                        <li>
                            <a @click="view = 'help'; helpPage = ''" :class="[view === 'help' ? 'is-active' : '']"><i
                                    class="fas fa-question-circle"/>&nbsp;&nbsp;Help</a>
                            <ul v-if="view === 'help'">
                                <li><a href='#' :class="[{'is-active': helpPage === 'tips&tricks'}]"
                                       @click="helpPage = 'tips&tricks'"><i class="fas fa-lightbulb"/>&nbsp;&nbsp;Tips
                                    and tricks</a></li>
                                <li><a href='#' :class="[{'is-active': helpPage === 'gameWontStart'}]"
                                       @click="helpPage = 'gameWontStart'"><i class="fas fa-gamepad"/>&nbsp;&nbsp;Game
                                    won't start</a></li>
                                <li><a href='#' :class="[{'is-active': helpPage === 'modsNotWorking'}]"
                                       @click="helpPage = 'modsNotWorking'"><i class="fas fa-ban"/>&nbsp;&nbsp;Mods
                                    aren't working</a></li>
                                <li><a href='#' :class="[{'is-active': helpPage === 'likeR2'}]"
                                       @click="helpPage = 'likeR2'"><i class="fas fa-heart"/>&nbsp;&nbsp;Like r2modman?</a>
                                </li>
                            </ul>
                        </li>
                    </ul>
                                ...
                                <expandable-card
                                    ...
                                    <span class='card-footer-item'>
                                            ...
                                            <link-component :url="`${key.websiteUrl}${key.versionNumber}`"
                                                            :target="'external'">{{key.versionNumber}}</link-component>
                                        </span>
                                    ...
                                    <a class='card-footer-item' v-if="getMissingDependencies(key).length > 0"
                                       @click="downloadDependency(getMissingDependencies(key)[0])">Download
                                        dependency</a>
                                </expandable-card>
                            </div>
                        </template>
                    ...
                    <template>
                        ...
                        <ul class="list">
                            ...
                            <li class="list-item" @click="fixPreloader()">
                                <a class="is-text is-text--bold"><p class='has-tooltip-top'
                                                                    data-tooltip='This will attempt to fix any preloader errors.'>
                                    Run preloader fixer</p></a>
                            </li>
                    ...
                    <template>
                        ...
                        <div v-if="helpPage === 'tips&tricks'">
                            ...
                            <p>You can have multiple installs of r2modman, each one pointing to a different server, and
                                of course, one for your regular modded game.</p>
                            <br/>
                        ...
                        <div v-else-if="helpPage === 'modsNotWorking'">
                            ...
                            <p>If there is a missing dependency, you'll be able to resolve it by either reinstalling the
                                mod, or by clicking the "Download dependency" button.</p>
                            <br/>
                            ...
                            <p>You can update mods by going to the "Installed" tab, clicking on mods with the update
                                icon, and clicking the update button.</p>
                        ...
                        <div v-else-if="helpPage === 'likeR2'">
                                ...
                                <ul>
                                    <li>Leave a thumbs-up on
                                        <link-component url='https://thunderstore.io/package/ebkr/r2modman/'
                                                        :target="'external'">r2modman's Thunderstore page
                                        </link-component>
                                        .
                                    </li>
                            ...
                            <p>But most importantly, recommend new feature ideas! r2modman needs your help to be the
                                best possible mod manager for Risk of Rain 2!</p>
                        </div>
                        <div v-else>
                            ...
                            <p>Go to the "Online" tab, find a mod, and hit download. It'll also download the
                                dependencies, saving you time.</p>
