    ...
    <Teleport to="#activity-bar__left">
        <div class="activity-bar__group">
            <button class="activity-bar__context-item" id="game-switch-button" @click.prevent.stop="changeGame">
                <img class="game-icon" :src="imageSrc" :alt="t('translations.pages.manager.navigation.profileSwitcher.gameIconAltText')"/>
        ...
        <div class="activity-bar__group">
            <ActivityDropdown>
                <button class="activity-bar__action">
                    ...
                    <span>{{ t('translations.pages.manager.navigation.activityBar.exportProfile') }}</span>
                </button>
                <template #popper>
                    <ul class="menu-list">
                        <li><a v-close-popper @click="store.dispatch('profileExport/exportProfileAsCode')"><i class="fa fa-fw fa-code icon--margin-right"></i>{{ t('translations.pages.manager.navigation.activityBar.exportToCode') }}</a></li>
                        <li><a v-close-popper @click="store.dispatch('profileExport/exportProfileAsFile')"><i class="fa fa-fw fa-file-download icon--margin-right"></i>{{ t('translations.pages.manager.navigation.activityBar.exportToFile') }}</a></li>
                    </ul>
                </template>
            </ActivityDropdown>
        </div>
