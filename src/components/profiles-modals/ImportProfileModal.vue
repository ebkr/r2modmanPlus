...
export default class ImportProfileModal extends Vue {
    ...
    async profileSelectOnChange(event: Event) {
        if (event.target instanceof HTMLSelectElement) {
            await this.$store.dispatch('profiles/setSelectedProfile', {profileName: event.target.value, prewarmCache: false});
        }
    }
    ...
    // Handles the importing of the profile (doesn't matter if it is from file or code)
    // When creating:
    ...
    //          2. Deletes the old profile (including the folder on disk)
    ...
    async importProfileHandler(files: string[] | null) {
        ...
        if (read !== null) {
            ...
            const parsed: ExportFormat = this.parseYamlToExportFormat(read);
    ...
    parseYamlToExportFormat(read: string) {
    ...
    async readProfileFile(file: string): Promise<string | null> {

        let read = '';

        if (file.endsWith('.r2x')) {
            read = (await fs.readFile(file)).toString();
...
<template>
    <ModalCard :is-active="isOpen" @close-modal="closeModal">
        ...
        <template v-slot:header v-else-if="importingProfile !== false">
        ...
        <template v-slot:body v-else-if="importingProfile !== false">
