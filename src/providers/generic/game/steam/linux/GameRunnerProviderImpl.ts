...
export default class GameRunnerProviderImpl extends GameRunnerProvider {
    ...
    public async startModded(game: Game, profile: Profile): Promise<void | R2Error> {
        ...
        return this.start(game, `--doorstop-enable true --doorstop-target "Z:${await FsProvider.instance.realpath(path.join(profile.getPathOfProfile(), "BepInEx", "core", "BepInEx.Preloader.dll"))}"`);
    }
